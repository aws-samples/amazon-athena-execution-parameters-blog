...
const listPreparedStatements = () => {
  ...
  console.log("Retrieving a list of workgroups and named queries");
  axios
    .get(API_URL + "/named-queries", config)
    .then((response) => {
...
const getData = () => {
  ...
  // TODO: Pull values from dynamic list of query parameters since we're no longer using single prepared statement
  const data = JSON.stringify({
    type: "standard",
    statement: selectedQueryString.value,
    parameters,
  ...
  axios
    ...
    .then((response) => {
      ...
      if (!response.data["ResultSet"]) {
        ...
        console.log("SQLStatementForm is emitting event");
...
<template>
  <section>
    <h2>Parameterized SQL Statements</h2>
    <hr />
    <form>
      <div>
      ...
      <div v-show="selectedQueryString">
        <h3>Query string of SQL statement:</h3>
        <p class="snippet">
          {{ selectedQueryString }}
        </p>
      </div>
